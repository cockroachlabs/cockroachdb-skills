...
ccloud cluster networking allowlist list <cluster-id>
```
...
ccloud cluster networking allowlist create <cluster-id> \
  --cidr-address "203.0.113.0/24" --sql --ui --name "Office"

ccloud cluster networking allowlist create <cluster-id> \
  --cidr-address "198.51.100.0/24" --sql --ui --name "VPN"
...
# Add as /32 (single IP)
ccloud cluster networking allowlist create <cluster-id> \
  --cidr-address "$(curl -s https://checkip.amazonaws.com)/32" \
